...
This works with Rails 3 Beta 3.
This is a fork of latimes/craken.
It has been packaged as a Gem using Jeweler.

Requires jeweler to build the gem.

To install
  1. clone this project
  2. run rake gem:install
  3. in your rails 3 Gemfile add> gem 'craken'

To use craken from capistrano just add the following to your Capfile:
  require 'craken/recipes'

This sets up before and after deploy hooks to run craken:uninstall and craken:install.
Before you used to have to manually add these to your Capistrano scripts.

Note: Capistrano tasks have not been tested yet.
...
You can use the special strings using the following format:
  special_job:
    special: @daily
    command: run_me_daily > /tmp/thing_to_do.log 2>&1
